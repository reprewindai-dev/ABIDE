...
Boundaries: VCCP/UCH/UCR = capability lifecycle; cAPI/Covenant = governed connection/discovery; CAPPO = fail-closed authorization; **ABIDE = blueprint and bounded execution-contract compilation**; Lockerphycer = secret/key domain; BYOS = execution substrate; Gnomledger/PGL = evidence/provenance/lineage; VNP = measurement; RepoGate = intake/security; Veklom ID = identity evidence; x402 = settlement where verified.

ABIDE may be sold as its own standalone product with its own workbench. Inside Capability OS, reuse ABIDE’s blueprint/contract capabilities and build a Veklom-native surface; do not embed the standalone ABIDE page wholesale.
